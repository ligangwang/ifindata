...
class WatchlistProvider extends ChangeNotifier {
  ...
  void _initializeWatchlist() {
    ...
    _watchlistService.getWatchlistStream().listen((symbols) {
  ...
  Future<void> _loadWatchlistData() async {
    ...
    try {
      ...
      _watchlistData = await _watchlistService.getWatchlistWithData();
    } catch (e) {
  ...
  Future<bool> addToWatchlist(String symbol) async {
    try {
      ...
      final success = await _watchlistService.addToWatchlist(symbol);

      if (success) {
        // The stream will automatically update the list
        return true;
      } else {
        _errorMessage = 'Failed to add to watchlist';
        return false;
      }
    } catch (e) {
  ...
  Future<bool> removeFromWatchlist(String symbol) async {
    try {
      ...
      final success = await _watchlistService.removeFromWatchlist(symbol);
