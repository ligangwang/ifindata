// lib/providers/mock_stock_provider.dart
import 'package:flutter/material.dart';
import '../models/stock_data.dart';
import '../services/mock_stock_service.dart';

class MockStockProvider extends ChangeNotifier {
  ...
  Future<void> loadMarketIndices() async {
    try {
      ...
      _marketIndices = await MockStockService.getMarketIndices();
    } catch (e) {
  ...
  Future<void> loadTrendingStocks() async {
    try {
      ...
      _trendingStocks = await MockStockService.getTrendingStocks();
    } catch (e) {
  ...
  Future<void> searchStocks(String query) async {
    ...
    try {
      ...
      final symbols = await MockStockService.searchStocks(query);
      _searchResults = [];

      // Get stock data for search results
      for (final symbol in symbols.take(10)) {
        // Limit to 10 results
        final stockData = await MockStockService.getStockQuote(symbol.symbol);
        if (stockData != null) {
  ...
  Future<void> getStockQuote(String symbol) async {
    try {
      ...
      _selectedStock = await MockStockService.getStockQuote(symbol);
    } catch (e) {
  ...
  Future<void> loadHistoricalData(
    ...
    try {
      ...
      _historicalData = await MockStockService.getHistoricalData(
        symbol,
        startDate: startDate,
        endDate: endDate,
      );
